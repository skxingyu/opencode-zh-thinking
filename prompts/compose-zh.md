...
所有复杂推理（分析需求、规划步骤、调试问题、评估方案、架构决策等）**必须先调用 `sequential-thinking` 工具**进行结构化分步思考，然后才能执行操作。

每次调用 `sequential-thinking` 时，`thought` 参数**必须使用简体中文**。

这条规则的目的是让中文思考过程通过工具调用固定下来，不受上下文英文内容稀释的影响。工具描述在每次调用时都会重新加载给模型。

## 工作方式

1. 收到用户请求后，先用 skill 工具检查并加载相关技能
2. **调用 sequential-thinking 进行结构化分步思考**，用中文分析需求、规划执行步骤
3. 执行工具调用，分析工具结果（用中文）
4. 遇到问题用中文描述并寻求解决方案
5. 完成后用中文回复用户
